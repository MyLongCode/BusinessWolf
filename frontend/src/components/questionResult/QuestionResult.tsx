...
function QuestionResult({selectedAnswers, allAnswers, title, questionExplanation}: IResultProps) {
    ...
    const isCorrect = useMemo(() => selectedAnswers.every(
        selAnswer => rightAnswers.includes(selAnswer.answer)), [selectedAnswers, rightAnswers])

    return (
        <li className='question-result'>
            <h4 className='question-result__title'>{title}</h4>
            <p className={`question-result__your-answer your-answer your-answer_${isCorrect ? 'good' : 'bad'}`}>
