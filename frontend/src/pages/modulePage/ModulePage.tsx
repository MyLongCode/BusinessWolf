        ...
        <ModuleLayout headerTitle={`Модуль ${id}`}>
            <div className='module-page'>
                <SliderButton className={'module-page__slider-btn'} buttonNames={{left: "Коспекты", right: "Тесты"}}
                              ...
                              currentState={stateChanged && initialState === States.lessons ? "right" : stateChanged ? "left" : ""}
                              buttonClickHandler={
                                  state => sliderButtonClickHandler(state === "left" ? States.lessons : States.tests)
                              }/>
                <motion.div
                ...
                >
                    {initialState === States.lessons && <LessonsList moduleID={Number(id)} lessons={lessons}/>}
                    {initialState === States.tests && <TestsList moduleID={Number(id)} tests={tests} lessons={lessons}/>}
                </motion.div>
            </div>
        </ModuleLayout>
    );
