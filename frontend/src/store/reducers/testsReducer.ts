...
export const testsReducer = (state = initialState, action: TestsAction): TestState => {
    switch (action.type) {
        ...
        case TestsActionTypes.PUSH_TEST_ERROR:
            return {loading: false, error: 'Ошибка при отправке ответов', questions: state.questions}
        default:
            return state;
    }
}